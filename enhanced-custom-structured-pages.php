...
class Enhanced_CSP_Plugin {
    ...
    add_action('admin_enqueue_scripts', function() {
        wp_enqueue_style('custom-cms-admin', plugins_url('admin/custom-admin.css', __FILE__));
    });
}
...
public function update_database_schema() {
    ...
    $category_table = $wpdb->prefix . 'enhanced_csp_pages_categories';
    ...
    if ($current_version !== self::VERSION) {
        // Update main table column types to ensure consistency
        $column_updates = [
            'id' => 'BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT',
            'slug' => 'VARCHAR(191) NOT NULL',
            'status' => 'VARCHAR(20) NOT NULL DEFAULT "draft"',
        ...
        foreach ($indexes as $index) {
            $index_name = strtolower(preg_replace('/[^a-zA-Z0-9_]/', '', explode(' ', $index)[1]));
            $index_exists = $wpdb->get_results("SHOW INDEX FROM `{$table_name}` WHERE Key_name = '{$index_name}'");
            
            if (empty($index_exists)) {
                $wpdb->query("ALTER TABLE `{$table_name}` ADD {$index}");
            }
        }
        ...
        foreach ($category_indexes as $index) {
            $index_name = strtolower(preg_replace('/[^a-zA-Z0-9_]/', '', explode(' ', $index)[1]));
            $index_exists = $wpdb->get_results("SHOW INDEX FROM `{$category_table}` WHERE Key_name = '{$index_name}'");
            
            if (empty($index_exists)) {
                $wpdb->query("ALTER TABLE `{$category_table}` ADD {$index}");
            }
        }
...
public function get_endpoint_args_for_item_schema() {
    return [
        ...
        'faq_heading' => ['type' => 'string', 'required' => false],
...
public function save_page(WP_REST_Request $req) {
    ...
    $data = [
        ...
        'faq_heading' => isset($params['faq_heading']) ? sanitize_text_field($params['faq_heading']) : null,
    ...
    if (empty($params['id'])) {
        ...
        if ($exists) {
            return new WP_Error('duplicate_slug', 'A page with this slug already exists.', ['status' => 400]);
        }
    }
